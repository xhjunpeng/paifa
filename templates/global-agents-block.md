...
High-risk work at its risk floor may be dispatched without additional user confirmation. Explicit user confirmation is required only for work above Sol high, irreversible operations, or changes that increase high-risk consequences. `PAIFA_DISPATCHED` must record actual model, effort, and context values. Each actual value must match its corresponding `PAIFA_ROUTE` value.
